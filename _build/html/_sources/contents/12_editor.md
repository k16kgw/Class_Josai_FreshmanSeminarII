...
| コマンド名 | 挙動 |
| :--      | :-- |
| `pwd`    | 現在いるフォルダの場所を出力する． |
| `ls`     | 現在いるフォルダに含まれている**ファイル・フォルダの一覧を出力**する． |
| `ls -l`  | `ls`を詳細も含めて出力する． |
...
- **絶対パス**：`/Users/username/Documents/...`
- **相対パス**：`./`, `../`

cd + ls + pwd を組み合わせると迷子にならない
...
- ターミナルから Python を実行する
- `.py` ファイルを直接動かす
- Jupyter を使わない実行方法

```{note}
次回は「**Pythonを本当の意味で使う回**」です。
````
